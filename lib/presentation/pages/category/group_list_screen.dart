...
class _GroupListScreenState extends State<GroupListScreen> {
  final _service = CategoryRepositoryImpl();
                          ...
                          content: SizedBox(
                            ...
                            child: ListView(
                              ...
                              children: group.memberUserIds.map((id) {
                                try {
                                  final user = AuthRepositoryImpl().users
                    ...
                    onPressed: () {
                      ...
                      if (role != 'student') {
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(
                            content: Text(
                              'Solo estudiantes pueden unirse a grupos',
                            ),
                          ),
                        );
                        return;
                      ...
                      final status = _service.addMemberToGroup(
                        widget.categoryId,
                        group.id,
                        currentUser.id,
                      );
                      if (status == 1) {
                        ...
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(
                            content: Text('No se pudo unir al grupo'),
                          ),
                        );
                      }
                    },
